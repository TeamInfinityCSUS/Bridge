...
if ($result->num_rows > 0) { //Checks if Query table is empty
    while($row = $result->fetch_assoc()) { // Fetches the first row
        ...
        $result_array .= 'First Name:<input type="text" class="form-control" id="editing2" placeholder="' . $row['fname'] . '">';
        $result_array .= 'Last Name:<input type="text" class="form-control" id="editing1" placeholder="' . $row['lname'] . '">';
        $result_array .= 'Account Type<input type="text" class="form-control" id="editing5" placeholder="' . $row['acc_type'] . '">';
